...
DOMAIN = 'jarolift'
Counter_Filename=os.path.join(os.path.expanduser("~"),'.homeassistant','mycounter.txt');
...
def setup(hass, config):
    ...
    def handle_send_raw(call):
        ...
        hass.services.call('remote', 'send_command', {'entity_id':remote_entity_id, 'command':[packet]})
    ...
    def handle_clear(call):
        ...
        Button = int('0x4',16);
        for i in range(0,6):
            packet = BuildPacket(Grouping,Serial,Button,UsedCounter+1+i,MSB,LSB,False);
            hass.services.call('remote', 'send_command', {'entity_id':remote_entity_id, 'command':[packet]})
            sleep(0.5);
        sleep(1);
        Button = int('0x8',16);
